...
foreach ($info as $c_nome => $p) {
    ...
    foreach ($p as $prod) {
        ...
        if (!in_array_indexed($keys_prod, $produtos))
            $produtos = array_merge($produtos, $keys_prod);
    }

    $series = array();

    foreach ($produtos as $prod_nome) {
        $data = "";
        foreach ($p as $_periodo) {
            $data .= ( isset($_periodo[$prod_nome]) && !empty($_periodo[$prod_nome])) ? "{$_periodo[$prod_nome]}" : 'null';
            $data .= " ,";
        }
