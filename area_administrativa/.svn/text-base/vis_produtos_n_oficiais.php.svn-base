<?php
	...
	$strsql = "SELECT COUNT(A.produto_id) AS QT,SUM(C.precos_total) AS TOT FROM tabela_produtos A, tabela_unidade_medidas B,tabela_precos C WHERE (A.produto_id = C.produto_id AND B.medida_id = C.medida_id) AND C.coleta_id = '".$coleta_id."' AND A.produto_cesta = '0' GROUP BY C.coleta_id";   
	
 	$produtos = mysql_query($strsql) or die(mysql_error());
	$row = mysql_fetch_array($produtos);
	...
	$strsql = "SELECT * FROM tabela_produtos A, tabela_unidade_medidas B,tabela_precos C WHERE (A.produto_id = C.produto_id AND B.medida_id = C.medida_id) AND coleta_id = '".$coleta_id."' AND A.produto_cesta = '0' ORDER BY A.produto_id LIMIT ".$start_rec.",".$records_per_page;
	...
	<table cellspacing="0" id="listTable" summary="Tabela de Preços das Coletas" style="width:655px;">
		...
		?>
			<tr class="<?php echo ($l_cor);?>">
				...
				<td class="tdboder"><?php echo($row['medida_simbolo']); ?> (<?php echo($row['medida_descricao']); ?>)</td>
				<?php
				...
				$resultados = mysql_query($strsql) or die(mysql_error()); 
				$res = mysql_fetch_array($resultados);
				$total_n_oficiais += $res['precos_total'];
				?>
	...
	<table align="left" style="border:0; margin-top:10px;" width="585px;">
		<tr>
			<td align="left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<?php if ($start_rec!=0) {?>
			<span class="pag_links"><a href="javascript: void(0)" onClick="atualiza_tabela('0','produtos_n_oficiais',<?php echo($coleta_id); ?>);" >Primeira</a></span>&nbsp;&nbsp;|&nbsp;<?php }else{ ?> <span class="pag_links">Primeira</span>&nbsp;&nbsp;|&nbsp;<?php }?>
			<?php if ($back_rec>=0) {?>
			<span class="pag_links"><a href="javascript: void(0)" onClick="atualiza_tabela('<?php echo($back_rec); ?>','produtos_n_oficiais',<?php echo($coleta_id); ?>)">Anterior</a></span>
			<?php }else{?>
			...
			<span class="pag_links"><a href="javascript:void(0)" onClick="atualiza_tabela('<?php echo($last_rec); ?>','produtos_n_oficiais',<?php echo($coleta_id); ?>)" >Pr&oacute;xima</a></span>&nbsp;&nbsp;|&nbsp;<?php }else {?>
			<span class="pag_links">Pr&oacute;xima</span>&nbsp;&nbsp;|&nbsp;
			<?php } ?>
			<?php if ($start_rec+$records_per_page<$total_rec) {?>
			<span class="pag_links"><a href="javascript: void(0)" onClick="atualiza_tabela('<?php echo($start_last_page); ?>','produtos_n_oficiais',<?php echo($coleta_id); ?>)">&Uacute;ltima</a></span>
			<?php }else{ ?>
...
{
?>
	<h3 style="text-align:left">&nbsp;&nbsp;&nbsp;&nbsp;Sem produtos n&atilde;o oficiais cadastrados</h3>
<?php 
}
?>