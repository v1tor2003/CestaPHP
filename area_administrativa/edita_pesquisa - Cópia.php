<?php
	...
	if($situacao != 0)
	{
		...
		// Verifica - se a exist�ncia de coletas pertencentes a pesquisa ainda n�o encerradas
			...
			if($res && mysqli_num_rows($res)>0)
				while ($row = mysqli_fetch_array($res))
				{
					...
					if($res1 && mysqli_num_rows($res1)>0)
					{
						...
						if($oficial == 1)
						{
						$str = "SELECT A.pesquisa_id,A.cidade_id,A.gasto_mensal_cesta FROM tabela_pesquisas_cidades A,tabela_pesquisas B WHERE EXTRACT(MONTH FROM B.pesquisa_data) = '".$data_mes_anterior[0]."' AND EXTRACT(YEAR FROM B.pesquisa_data) = '".$data_mes_anterior[1]."' AND A.pesquisa_id = B.pesquisa_id AND A.cidade_id = '".$cidade_id."' UNION ALL SELECT A.pesquisa_id,A.cidade_id,A.gasto_mensal_cesta FROM tabela_pesquisas_cidades A,tabela_pesquisas B WHERE EXTRACT(MONTH FROM B.pesquisa_data) = '".$data_mes_semestre[0]."' AND EXTRACT(YEAR FROM B.pesquisa_data) = '".$data_mes_semestre[1]."' AND A.pesquisa_id = B.pesquisa_id AND A.cidade_id = '".$cidade_id."' UNION ALL SELECT A.pesquisa_id,A.cidade_id,A.gasto_mensal_cesta FROM tabela_pesquisas_cidades A,tabela_pesquisas B WHERE EXTRACT(MONTH FROM B.pesquisa_data) = '".$data_ano_anterior[0]."' AND EXTRACT(YEAR FROM B.pesquisa_data) = '".$data_ano_anterior[1]."' AND A.pesquisa_id = B.pesquisa_id AND A.cidade_id = '".$cidade_id."'";
						
						$res_v = mysqli_query($conn, $str) or die(mysqli_error($conn));
						$tam = mysqli_num_rows($res_v);
						
						for($i = 0; $i<$tam ; $i++)
						{
							$row_v = mysqli_fetch_array($res_v);
							$variacao[$i] = round_valor((($gasto_mensal_cesta/$row_v['gasto_mensal_cesta'])-1)*100,2);
