...
<table cellspacing="0" id="listTable" style="width:650px; font-size:11px;">
	<colgroup>
		<col id="codigo" />
		<col id="nome" />
		<col id="cidade" />
	...
	<thead>
		<tr>
			...
			<th scope="col" class="tdboderCod">Pre&ccedil;o M&eacute;dio</th>
			<th scope="col" class="tdboderCenter">Gasto Mensal</th>
...
?>
		...
		<td class="tdboder"><?php echo(formata_numero($row['racao_minima_quantidade'])." (".$row['medida_simbolo'].")"); ?></td>
		<td class="tdboderCod"><?php echo(formata_numero($row['produto_preco_medio'])); ?></td>
		<td class="tdboderCenter"><?php echo(formata_numero($row['produto_preco_total'])); ?></td>
		<td class="tdboderCenter" <?php if($row['produto_variacao_mensal']<0){?> style="color:#FF3300;" <?php }?> ><?php if($row['produto_variacao_mensal'] == '')echo(" - ");else echo(formata_numero($row['produto_variacao_mensal'])); ?></td>
		<td class="tdboderCenter" <?php if($row['produto_variacao_semestral']<0){?> style="color:#FF3300;" <?php }?> ><?php if($row['produto_variacao_semestral'] == NULL)echo("  - ");else echo(formata_numero($row['produto_variacao_semestral'])); ?></td>
		<td class="tdboderCenter" <?php if($row['produto_variacao_anual']<0){?> style="color:#FF3300;" <?php }?> ><?php if($row['produto_variacao_anual'] == NULL)echo("  - ");else echo(formata_numero($row['produto_variacao_anual'])); ?></td>
		<?php 
...
<tr>
		...
		<td style="border:#FFFFFF; text-align:left; background-color:#CCCCCC;" align="left">
		...
		$variacao_anual = ($row['variacao_anual']!=''?$row['variacao_anual']:'-');
