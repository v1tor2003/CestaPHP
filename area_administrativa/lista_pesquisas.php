<?php 
	...
	if($action == 'filtrar')
	{
		...
		$strsql = "SELECT A.salario_id,A.pesquisa_id,B.mes_nome,EXTRACT(YEAR FROM A.pesquisa_data) AS pesquisa_ano,pesquisa_fechada, S.salario_id,S.salario_valor_liquido,S.salario_valor_bruto FROM tabela_pesquisas A LEFT OUTER JOIN tabela_salarios S ON A.salario_id = S.salario_id,tabela_mes B  WHERE A.pesquisa_detalhada = '1' AND EXTRACT(MONTH FROM A.pesquisa_data) = B.mes_id AND ".$campo_filtrado." ".$operador_filtro." '".$valor_filtro."' ORDER BY ".$ordenar_por[0]." ".$ordenar_por[1];
		
	}
	else
	{
		$strsql = "SELECT A.salario_id,A.pesquisa_id,B.mes_nome,EXTRACT(YEAR FROM A.pesquisa_data) AS pesquisa_ano,pesquisa_fechada,S.salario_id,S.salario_valor_liquido,S.salario_valor_bruto FROM tabela_pesquisas A LEFT OUTER JOIN tabela_salarios S ON A.salario_id = S.salario_id,tabela_mes B  WHERE A.pesquisa_detalhada = '1'  AND EXTRACT(MONTH FROM A.pesquisa_data) = B.mes_id ORDER BY A.pesquisa_data DESC";
	}
...
?>		
<table  border="0px" id="link_table_add"  style="width:590px"><tr><td>[<a href="add_pesquisas.php" id="link_adicionar" >Adicionar</a>]</td></tr></table>
			...
			<!-- Contedo referente a esta pgina -->
					...
					<thead>
						<tr>
							...
							<th scope="col" colspan="5" class="tdboder">A&ccedil;&otilde;es</th>
						</tr>
					</thead>
						 <?php
...
<td class="tdboderCod">
	<a href="javascript: void(0)" onClick="return submit_Action_Check('<?php echo($row['pesquisa_id']); ?>','edit','','','add_pesquisas.php','A pesquisa de <?php echo($row['mes_nome']."/".$row['pesquisa_ano']); ?> esta fechada!');" id="<?php echo($id_editar); ?>" ><img src="<?php echo $img_editar; ?>" border="0" /></a>
</td>

<td class="tdboderCod">
	<a href="javascript: void(0)" onClick="return submit_Action_Check('<?php echo($row['pesquisa_id']); ?>','del','<?php echo($row['mes_nome']."/".$row['pesquisa_ano']); ?>','Deseja apagar a pesquisa ','add_pesquisas.php','A pesquisa de <?php echo($row['mes_nome']."/".$row['pesquisa_ano']); ?> esta fechada!');" id="<?php echo($id_excluir); ?>" ><img src="<?php echo($img_excluir); ?>" border="0"/></a>
</td> 

<td class="tdboderCod">
	<a href="javascript: void(0)" id="<?php echo($id_carrinho); ?>" onClick="return submit_Action_Check('<?php echo($row['pesquisa_id']); ?>','','','','cadastro_coletas.php?pesquisa_id=<?php echo($row['pesquisa_id']); ?>','A pesquisa de <?php echo($row['mes_nome']."/".$row['pesquisa_ano']); ?> esta fechada!');" ><img src="<?php echo($img_carrinho); ?>" border="0"/></a>
</td>
<td class="tdboderCod">
<a id="<?php echo($id_visualizar); ?>" onClick="return submit_Action_Check1('<?php echo($row['pesquisa_id']); ?>','','','','visualizacao_pesquisa.php?pesquisa_id=<?php echo($row['pesquisa_id']); ?>','A pesquisa de <?php echo($row['mes_nome']."/".$row['pesquisa_ano']); ?> ainda nao foi fechada!');" ><img src="<?php echo($img_visualizar); ?>" border="0"/></a>
</td>
